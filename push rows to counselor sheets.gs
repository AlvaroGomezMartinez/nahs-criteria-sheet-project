...
function pushRowsToCounselorSheet() {
  ...
  var values = wsActive
    .getRange(2, 1, wsActive.getLastRow() - 1, wsActive.getLastColumn())
    .getValues();
  var hensleyCaseLoad = [];
  ...
  wsHensley
    .getRange(2, 1, hensleyCaseLoad.length, hensleyCaseLoad[0].length)
    .setValues(hensleyCaseLoad);
  wsRodriguez
    .getRange(2, 1, rodriguezCaseLoad.length, rodriguezCaseLoad[0].length)
    .setValues(rodriguezCaseLoad);
}
...
const wsRodriguezRange = wsRodriguez.getRange(
  2,
  4,
  wsRodriguez.getLastRow() - 1,
  1,
);
